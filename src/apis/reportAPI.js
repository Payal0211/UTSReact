...
export const ReportAPI = {
	...
	getJoiningRevenueDataRequest: async function (reportData) {
		let httpService = new HttpServices();
		httpService.URL =
			NetworkInfo.NETWORK +
			'UTSReports/' +
			'Get_WeeklyCGR_JoiningRevenue' +
			reportData;
		httpService.setAuthRequired = true;
		// httpService.dataToSend = reportData;
		httpService.setAuthToken = UserSessionManagementController.getAPIKey();
...
getALLPotentialClosuresCommentsAPI:async function (payload) {
	...
	getPOCPopupReportAPI:async function (payload) {
		...
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.REPORT +
			ReportType.GET_POC_DETAILS_LIST + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&month=${payload?.month}&year=${payload?.year}&stage_ID=${payload?.stageID}&weekno=${payload?.week}&category=${payload?.cat}&multiplePODIds=${payload?.multiplePODIds}`
...
getPOChrSummaryPopupReportAPI:async function (payload) {
	...
	getPOCPopupMultiMonthReportAPI:async function (payload) {
		...
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.REPORT +
			ReportType.GET_POD_DASHBOARD_MULTIPLE_MONTH_POUP + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&monthstr=${payload?.monthstr}&yearstr=${payload?.yearstr}&stage_ID=${payload?.stageID}&monthno=${payload?.monthno}&category=${payload?.category}&multiplePODIds=${payload?.multiplePODIds}`
	...
	getJoinedSummeryReportAPI:async function (payload) {
		let httpService = new HttpServices();
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.REPORT +
			ReportType.GET_JOINED_SUMMARY_REPORT + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&month=${payload?.month}&year=${payload?.year}`
		...
		try {
			...
			return errorDebug(error, 'ReportAPI.getJoinedSummeryReportAPI');
		}
	},
	getMonthStartingSummaryReportAPI:async function (payload) {
		let httpService = new HttpServices();
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.REPORT +
			ReportType.GET_MONTH_STARTING_SUMMARY_REPORT + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&month=${payload?.month}&year=${payload?.year}`
		...
		try {
			...
			return errorDebug(error, 'ReportAPI.getMonthStartingSummaryReportAPI');
		}
	},
	...
	getPODWiseQuarterlySummaryAPI:async function (payload) {
		...
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.REPORT +
			ReportType.GET_POD_WISE_SUMMARY + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&month=${payload?.month}&year=${payload?.year}`
...
getAllPODGroupUsersRequest:async function (payload) {
	...
	httpService.URL =
		NetworkInfo.NETWORK +
		SubDomain.REPORT +
		ReportType.GET_POD_DASHBOARD_USERS_GROUP_REPORT + `?PODGroupID=${payload.id}&month=${payload.month}&year=${payload.year}`
	...
	httpService.URL =
		NetworkInfo.NETWORK +
		SubDomain.REPORT +
		ReportType.GET_POD_DASHBOARD_LIST + `?hrmodel=${payload?.hrmodel}&pod_id=${payload?.pod_id}&month=${payload?.month}&year=${payload?.year}&multiplePODIds=${payload?.multiplePODIds}`
