...
export const HiringRequestAPI = {
	...
	createNEWHiringWithVariableEquityRequest: async function (hrData) {
		let httpService = new HttpServices();
		const miscData = UserSessionManagementController.getUserMiscellaneousData();
		...
		httpService.URL =
			NetworkInfo.NETWORK +
			SubDomain.HIRING +
			HiringRequestsAPI.CREATE_NEW_HR_WITH_ADV +
			`?LoggedInUserId=${miscData?.loggedInUserTypeID}`;
		httpService.dataToSend = hrData;
		httpService.setAuthRequired = true;
		httpService.setAuthToken = UserSessionManagementController.getAPIKey();
		try {
			...
			return errorDebug(error, 'HiringRequestAPI.createNEWHiringWithVariableEquityRequest');
		}
	},
