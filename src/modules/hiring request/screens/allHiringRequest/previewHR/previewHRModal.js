    ...
    if(!isFreshersAllowed && editBudget?.employmentType?.toLocaleLowerCase() !== 'part time'){
      if(editBudget.currency === 'INR'){
        if(Number(editBudget?.budgetFrom) < 10000){
           _errors.budgetFrom = "Minimum Budget should be at least 5 digits"
        valid = false;
        }
       
      }else{
        if(Number(editBudget?.budgetFrom) < 1000){
          _errors.budgetFrom = "Minimum Budget should be at least 4 digits"
          valid = false;
       }
      }
    }
...
async function onHandleBlurImage(content, field) {
  ...
  if(list.length>0){
    ...
    if(field === 'About company'){      
      setAboutCompanyValue(updatedContent)
    }

    if(field === 'Culture'){
      setCulture(updatedContent)
    }
                        ...
                        <div class="company-details-inner dtlQuillEdit">
                          ...
                          {!isAboutCompany ?
                            ...
                            <>
                              <ReactQuill
                                ...
                                className="heightSize"
                                onBlur={()=>onHandleBlurImage(aboutCompanyValue,'About company')}
                              />
                          ...
                          {isCulture &&
                            <ReactQuill
                              ...
                              onBlur={()=>onHandleBlurImage(culture,'Culture')}
                              modules={modules}
                            />
                          }
