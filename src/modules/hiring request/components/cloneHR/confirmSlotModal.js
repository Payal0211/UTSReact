...
const ConfirmSlotModal = ({ getConfirmSlotDetails, onCancel, handleSubmit, confirmSlotRadio, setConfirmSlotRadio, talentInfo, hrId, getDateNewFormate, setDateNewFormate, hiringRequestNumber, setHRapiCall, callHRapi }) => {
    ...
    const dateConverter = () => {
        ...
        const newDateFormate = getConfirmSlotDetails?.Slots?.map((item, index) => {
            ...
            return `Slot-${index + 1} ${days[date.getDay()]} ${monthNames[date.getMonth()]} ${date.getDate()}  ${date.getFullYear()} ${item?.strStartTime} ${item?.strStartTime?.slice(0, item?.strStartTime?.indexOf(":")) > 12 ? "PM" : "AM"} to ${item?.strEndTime}  ${item?.strStartTime?.slice(0, item?.strEndTime?.indexOf(":")) > 12 ? "PM" : "AM"}`
        })
    ...
    const saveConfirmSlotDetailsHandler = async (data) => {
        const shortListId = getConfirmSlotDetails?.Slots.filter((item, index) => confirmSlotRadio === index + 1)
        ...
        const response = await hiringRequestDAO.saveConfirmSlotDetailsDAO(confirmSlotPayload)
    ...
    return (
        <div
            className={confirmSlotStyle.interviewContainer}
        >
                    ...
                    <div className={confirmSlotStyle.row}>
                        ...
                        <div className={confirmSlotStyle.colMd4}>
                            <div className={confirmSlotStyle.transparentTopCard}>
                                ...
                                <div className={confirmSlotStyle.cardTitle}>{getConfirmSlotDetails?.InterviewRound}</div>
                            </div>
                        </div>
                    </div>
            ...
            <div className={confirmSlotStyle.formPanelAction}>
                ...
                <button
                    className={confirmSlotStyle.btn} onClick={() => onCancel()} >
                    Cancel
                </button>
            </div>
