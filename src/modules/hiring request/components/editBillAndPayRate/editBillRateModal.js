...
const EditBillRate = ({
    ...
    const saveBillRatehandler = async (data) => {
        ...
        const response = await hiringRequestDAO.updateHRCostRequestDAO(saveBillRatePayload)
    ...
    return (
        ...
        >
            <div className={` ${editBillAndPayRate.headingContainer} ${editBillAndPayRate.billRateContainer}`}>
                <h1>Edit Bill Rate</h1>
            </div>
            <div className={editBillAndPayRate.firstFeebackTableContainer}>
                <div className={`${editBillAndPayRate.row} ${editBillAndPayRate.billRateWrapper}`}>
                    <div
                        className={editBillAndPayRate.colMd12}>
                        <p>Currency<span>*</span> : {Object.keys(getBillRateInfo).length > 0 && getBillRateInfo?.currency_Sign}</p>
                        <p>Talent Fees<span>*</span> : {Object.keys(getBillRateInfo).length > 0 && `${getBillRateInfo?.talent_Fees} ${getBillRateInfo?.currency_Sign} / Month `}</p>
                    </div>
                    <div
                        className={editBillAndPayRate.colMd12}>
                        <HRInputField
