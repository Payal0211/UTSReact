...
const EngagementFilerList = ({
    ...
    useEffect(() => {
        getHTMLFilter ?
            setTimeout(() => {
                document.querySelector(`.${engagementFilterStyle.aside}`).classList.add(`${engagementFilterStyle.closeFilter}`)
            }, 300) :
            document.querySelector(`.${engagementFilterStyle.aside}`).classList.remove(`${engagementFilterStyle.closeFilter}`)
    }, [getHTMLFilter])
...
export default EngagementFilerList;
