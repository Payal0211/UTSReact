...
const IncentiveReportScreen = () => {
  ...
  const onRowClick = async (record) => {
    ...
    response.responseBody[0]?.userRole === "AM Head" )
      {
        setValueOfSelected("AM");
        let _inValues = [...incentiveReportColumn];
        let data = _inValues?.filter((i)=>{
          return i?.title === "AM NR Slab" || i?.title === "AM NR %"
        })
        if(data.length<1){
          setValueOfSelected("AM NR Slab");
          _inValues.push({title:"AM NR Slab",dataIndex:"AMNRSlab"},{title:"AM NR %",dataIndex:"AMNRPercentage"})
          setIncentiveReportColumn(_inValues);
        }
        // console.log(_inValues,"Invalues");
      }
     else if( response.responseBody[0]?.userRole === "POD Manager" ||
    response.responseBody[0]?.userRole === "Sales Consultant" )
      {
        setValueOfSelected("POD Manager");
        let _inValues = [...incentiveReportColumn];
        let data = _inValues?.filter((i)=>{
          return i?.title === "Slab" || i?.title === "Slab Amt ($)"
        })
        if(data.length<1){
          _inValues.push({title:"Slab",dataIndex:"Slab"},{title:"Slab Amt ($)",dataIndex:"SlabAmt"})
          setIncentiveReportColumn(_inValues);
        }
        console.log(_inValues,"Invalues");
      }
  ...
  return (
    <div className={IncentiveReportStyle.hiringRequestContainer}>
      ...
      incentiveReportInfo[0]?.userRole === "AM Head" ? (
       ...
       <Table
        columns={valueOfSelected === "AM" ? test1  : valueOfSelected === "AM NR Slab" ? test2 : test3
        }
