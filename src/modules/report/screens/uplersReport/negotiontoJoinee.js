    ...
    if (joinedResult.statusCode === HTTPStatusCode.OK) {
      setJoiningSummaryData(
        joinedResult && [...joinedResult?.responseBody, {
          ...joinedResult?.responseBody[0], stage: 'Joining Projection',

          w1: joinedResult?.responseBody[0]?.total_JoinedProjection,
          w2: "",
          w3: "",
          w4: "",
          w5: "",
        }, {
          ...joinedResult?.responseBody[0], stage: 'Planning Projection',
          w1: joinedResult?.responseBody[0]?.total_PlannedProjection,
          w2: "",
          w3: "",
          w4: "",
          w5: "",
...
setLoadingResponse(false);
  ...
  const reportPlanningSummaryColumns = [
    {
      ...
      render: (val, row) => {
        ...
        if (val === 'Total Planned') {
          return <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>{val} {row?.total_AllweeksPlanned}</div>
        }
        //  if(val === 'Planning Projection'){
        //   return <div style={{display:'flex', flexDirection:'column',gap:'4px'}}>{val} {row?.total_PlannedProjection}</div>
        // }
        //  if(val === 'Joining Projection'){
        //   return <div style={{display:'flex', flexDirection:'column',gap:'4px'}}>{val} {row?.total_JoinedProjection}</div>
        // }
  ...
  {showFreeze && (
    ...
    <div className={uplersStyle.filterContainer} style={{ padding: "12px" }}>
      <div className={uplersStyle.filterSets} style={{ paddingLeft: "20px", padding: '10px' }}>
