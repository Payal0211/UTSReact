...
export const engagementRequestDAO = {
	...
	getReplacementDetailsDAO : async function (payload) {
		try {
			const replaceTalentResponse = await EngagementRequestAPI.getUpdateReplacementDetails(payload);
			if (replaceTalentResponse) {
				const statusCode = replaceTalentResponse['statusCode'];
				if (statusCode === HTTPStatusCode.OK) {
					const tempResult = replaceTalentResponse.responseBody;
			...
			return errorDebug(
				error,
				'engagementRequestDAO.getReplacementDetailsDAO',
			);
		}
	},
  ...
  viewOnboardFeedbackDAO: async function (onBoardID) {
	...
	getTalentOtherDetailsOtherListDAO: async function (payload) {
		try {
			const allBRPRListResult = await EngagementRequestAPI.getTalentOtherDetailsOtherList(
				payload
			);
			if (allBRPRListResult) {
				const statusCode = allBRPRListResult['statusCode'];
			...
			return errorDebug(error, 'hiringRequestDAO.getTalentOtherDetailsOtherListDAO');
		}
	},
	...
	updateTentativeJoiningDate: async function (payload) {
		try {
			const updateResult = await EngagementRequestAPI.updateTentativeJoiningDateAPI(payload);
			if (updateResult) {
				const statusCode = updateResult['statusCode'];
				if (statusCode === HTTPStatusCode.OK) {
					const tempResult = updateResult.responseBody;
			...
			return errorDebug(error, 'hiringRequestDAO.updateTentativeJoiningDate');
		}
	},
    updateOfferCTCUlpersFeesDAO:async function (payload) {
		try {
			const updateResult = await EngagementRequestAPI.updateOfferCTCUlpersFeesAPI(payload);
			if (updateResult) {
				const statusCode = updateResult['statusCode'];
				if (statusCode === HTTPStatusCode.OK) {
					const tempResult = updateResult.responseBody;
			...
			return errorDebug(error, 'hiringRequestDAO.updateOfferCTCUlpersFeesDAO');
		}
	},
